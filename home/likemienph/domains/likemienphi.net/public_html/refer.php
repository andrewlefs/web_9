<?php
$titleweb =  'Kiếm tiền online, Tăng like Facebook, tang like facebook';
include('header.php');
if(!$is_online || $site['refsys'] != 1){
	redirect('index.php');
	exit;
}
$refs = $db->GetNumRows($db->Query("SELECT id FROM `users` WHERE `ref`='".$data['id']."' AND `activate`='0'"));
?>
<div class="content">
	...
	<div style="height:193px">
		...
		<div class="aff_block">
            <div class="aff_content">              
                ...
                <center><input type="text" value="<?=WEB_DOMAIN?>/?ref=<?=$data['id']?>" onclick="this.select()" style="font-size:11px;padding:4px;width:270px;margin:0 auto;" readonly="true" /></center>
				...
				<table width="100%" border="0" cellpadding="3" cellspacing="1">
					<tr>
						<td valign="top" align="center">
							<img src="<?=WEB_DOMAIN?>/img/banner.png" border="0" />
						</td>
					</tr>
					<tr>    
						<td valign="top" align="center">
							<b><?=$lang['b_118']?></b><br>
							<textarea class="textarea" style="width:550px;height:60px" onclick="this.select()" readonly="true"><a href="<?=WEB_DOMAIN?>/?ref=<?=$data['id']?>" target="_blank"><img src="<?=WEB_DOMAIN?>/img/banner.png" alt="<?=$site['site_name']?>" border="0" /></a></textarea>
						</td>
					</tr>
					<tr>    
						<td valign="top" align="center">
							<b>BB Code</b><br>
							<textarea class="textarea" style="width:550px;height:30px" onclick="this.select()" readonly="true">[url=<?=WEB_DOMAIN?>/?ref=<?=$data['id']?>][img]<?=WEB_DOMAIN?>/img/banner.png[/img][/url]</textarea>                        
						</td>
					</tr>                   
				</table>
        </div>
