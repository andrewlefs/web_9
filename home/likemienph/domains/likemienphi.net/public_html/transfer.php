...
if(isset($_POST['submit']) && $timecheck < time() && $valid_transfer) {
	...
	if($data['coins'] < $amount){
	...
	}elseif($db->QueryGetNumRows("SELECT id FROM `users` WHERE `login`='".$username."' AND `banned`='0'") != 1){
		...
		$rid = $db->QueryFetchArray("SELECT id FROM `users` WHERE `login`='".$username."' AND `banned`='0'");
...
<table class="table">
	...
	<tbody>
<?
$sql = $db->Query("SELECT * FROM `c_transfers` WHERE `receiver`='".$data['id']."' ORDER BY date DESC LIMIT 10");
...
<?}}else{?>
	<tr><td colspan="4"><center><?=$lang['b_157']?></center></td></tr>
<?}?>
	</tbody>
</table>
</div>	
