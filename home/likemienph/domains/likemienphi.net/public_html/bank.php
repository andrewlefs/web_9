<?php
$titleweb =  'Nạp tiền vào tài khoản, Tăng like Facebook, tang like facebook';
include('header.php');
if(!$is_online){
	redirect('index.php');
	exit;
...
?>
	...
	<div class="infobox" style="width:250px;margin:10px auto;display:inline-block"><b><?=$lang['b_255']?>: <span style="font-weight:600;color:green"><?=get_currency_symbol($site['currency_code'])?> <?=$data['account_balance']?></span></b></div>
<div class="infobox t-left">
<center><div class="ucp_link<?=(!isset($_GET['convert']) && !isset($_GET['withdraw']) ? ' active' : '')?>" style="margin-right:5px;display:inline-block"><a href="bank.php">Add Funds</a></div><?if($site['convert_enabled'] == 1){?><div class="ucp_link<?=(isset($_GET['convert']) ? ' active' : '')?>" style="margin-right:5px;display:inline-block"><a href="bank.php?convert"><?=$lang['b_268']?></a></div><?}?><?if($site['allow_withdraw'] == 1){?><div class="ucp_link<?=(isset($_GET['withdraw']) ? ' active' : '')?>" style="margin-right:5px;display:inline-block"><a href="bank.php?withdraw"><?=$lang['b_97']?></a></div><?}?></center><hr>
<?=$msg?>
...
<form method="post">
	...
	<p>
		...
		<option value="chuyenkhoan">Chuyển khoản</option>
			<option value="viettel">Card Viettel</option>
<option value="Vinaphone">Card Vinaphone</option>
<option value="Mobilphone">Card Mobilphone</option>
		</select>
	</p>
	<p>
...
?>
	...
	<div class="infobox" style="width:250px;margin:10px auto;display:inline-block"><b><?=$lang['b_255']?>: <span style="font-weight:600;color:green"><?=get_currency_symbol($site['currency_code'])?> <?=$data['account_balance']?></span></b></div>
<div class="infobox t-left">
<center><div class="ucp_link<?=(!isset($_GET['convert']) && !isset($_GET['withdraw']) ? ' active' : '')?>" style="margin-right:5px;display:inline-block"><a href="bank.php"><?=$lang['b_289']?></a></div><?if($site['convert_enabled'] == 1){?><div class="ucp_link<?=(isset($_GET['convert']) ? ' active' : '')?>" style="margin-right:5px;display:inline-block"><a href="bank.php?convert"><?=$lang['b_268']?></a></div><?}?><?if($site['allow_withdraw'] == 1){?><div class="ucp_link<?=(isset($_GET['withdraw']) ? ' active' : '')?>" style="margin-right:5px;display:inline-block"><a href="bank.php?withdraw"><?=$lang['b_97']?></a></div><?}?></center><hr>
<?=$msg?>
<iframe src="/thanhtoan/index.php?ref_code=<?=$data['login']?> " style="width: 650px; height: 400px;border:none"></iframe>
...
<!-- thanh toan paypal-->
</br><div style=" margin-left:auto; margin-right:auto; text-align:center; background:#FF9933;">
<div> NẾU BẠN CÓ TÀI KHOẢN PAYPAL</BR>CHÚNG TÔI CHẤP NHẬN THANH TOÁN QUA PAYPAL</BR>PAYMENT VIA PAYPAL</BR>(CHỈ ÁP DỤNG CHO KHÁCH QUỐC TẾ)</div>
<form method="post">
    <p>
...
<h2 class="title" style="margin-top:20px"><?=$lang['b_257']?></h2>
	<table class="table">
		...
		<tbody>
<?
  $sql = $db->Query("SELECT id,money,gateway,date,paid,payer_email FROM `transactions` WHERE `user_id`='".$data['id']."' ORDER BY `date` DESC LIMIT 10");
  $num = $db->GetNumRows($sql);
  if($num == 0){ echo '<tr><td colspan="6" align="center"><b>'.$lang['b_250'].'</b></td><tr>';}else{
  $trans = $db->FetchArrayAll($sql);
  foreach($trans as $tran){
