...
if(isset($_GET['skip']) && is_numeric($_GET['skip'])){
	...
	if($db->GetNumRows($db->Query("SELECT site FROM `surfed` WHERE `site`='".$skip."' AND `user`='".$data['id']."'")) == 0){
		$db->Query("INSERT INTO `surfed` (user, site) VALUES('".$data['id']."', '".$skip."')");
	}
}
...
$sit['id'] = 0;
if($site['surf_type'] != 1){
	$sit = $db->FetchArray($db->Query("SELECT a.id, a.url, a.title, a.cpc FROM surf a LEFT JOIN users b ON b.id = a.user LEFT JOIN surfed c ON c.user = '".$data['id']."' AND c.site = a.id WHERE (a.confirm = '0' AND a.active = '0') AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site IS NULL AND a.user !='".$data['id']."')".$dbt_value." ORDER BY a.cpc DESC, b.premium DESC, RAND() LIMIT 1"));
}elseif(isset($_GET['sid']) && is_numeric($_GET['sid'])){
	$sid = $db->EscapeString($_GET['sid']);
	if($db->GetNumRows($db->Query("SELECT site FROM `surfed` WHERE `site`='".$sid."' AND `user`='".$data['id']."'")) == 0){
		$sit = $db->FetchArray($db->Query("SELECT a.id, a.url, a.title, a.cpc FROM surf a LEFT JOIN users b ON b.id = a.user LEFT JOIN surfed c ON c.user = '".$data['id']."' AND c.site = a.id WHERE a.id = '".$sid."' AND (a.confirm = '0' AND a.active = '0') AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site IS NULL AND a.user !='".$data['id']."')".$dbt_value." LIMIT 1"));
	}
}
...
if($sit['id'] > 0){
	$key = $surf_time+time();
	$sql = $db->Query("SELECT ses_key FROM `module_session` WHERE `user_id`='".$data['id']."' AND `page_id`='".$sit['id']."' AND `module`='surf'");
        ...
        <td id="surfbar_td" align="center" height="68" valign="top">
          ...
          <center>
            <table style="color:white;" width="100%" border="0" height="100%">
              <tr>
                <td width="380" style="background:url('/theme/pes/images/logo.png') center left no-repeat;">&nbsp;</td>
