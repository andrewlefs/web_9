<?php
$titleweb =  'Thành viên VIP, Tăng like Facebook, tang like facebook';
include('header.php');
if(!$is_online){
	redirect('index.php');
	exit;
...
if(isset($_POST['vip_coins'])){
	...
	$sql = $db->Query("SELECT * FROM `p_pack` WHERE `id`='".$pid."' AND `type`='1'");
	if($db->GetNumRows($sql) > 0){
		$vc_pack = $db->FetchArray($sql);
...
<div class="content"><h2 class="title"><?=$lang['b_247']?> <span style="float:right"><a href="vip.php"><?=$lang['b_08']?></a></span></h2>
	<table class="table">
		...
		<tbody>
<?
  $sql = $db->Query("SELECT * FROM `user_transactions` WHERE `user_id`='".$data['id']."' AND `type`='1' ORDER BY `date` DESC");
...
<?
	...
	if(isset($_POST['submit']) && isset($_POST['pid'])){
		...
		$sql = $db->Query("SELECT id,days,price FROM `p_pack` WHERE `id`='".$pid."'");
...
?>
<link href='http://fonts.googleapis.com/css?family=Open+Sans+Condensed:300,300italic,700' rel='stylesheet' type='text/css' /><link rel="stylesheet" type="text/css" href="theme/pes/vip.css" />
<div class="content"><h2 class="title"><?=$lang['b_08']?> <span style="float:right"><a href="vip.php?history"><?=$lang['b_247']?></a></span></h2><?=$mesaj?> 
<div class="infobox t-left"><b><?=$lang['b_192']?>:</b> <?=($data['premium'] > 0 ? $lang['b_194'] : $lang['b_193'])?><?if($data['premium'] > 0){?><br /><b><?=$lang['b_159']?>:</b> <?=date('d-m-Y H:m', $data['premium'])?><?}?></div>
	<div class="vip_t_row">
        ...
        <div class="last_child">
			...
			<form method="POST">
				<span class="pack_select">
					<select name="pid">
					<?
						$cp_check = $db->GetNumRows($db->Query("SELECT id FROM `p_pack` WHERE `type`='1'"));
						foreach($db->QueryFetchArrayAll("SELECT id,days,price,coins_price FROM `p_pack` WHERE `type`='0' ORDER BY `price` ASC") as $pack){
		...
		<div class="last_child">
			...
			<form method="POST">
				<span class="pack_select">
					...
					<?
						foreach($db->QueryFetchArrayAll("SELECT id,days,coins_price FROM `p_pack` WHERE `type`='1' ORDER BY `coins_price` ASC") as $pack){
